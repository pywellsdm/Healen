...
import {
  ...
  Moon, Plus, Pencil, Camera, ChevronLeft, Users,
} from "lucide-react";
...
const TABS = [
  { key: "ai", label: "AI", desc: "Provider, coach persona & characters", icon: Bot },
  { key: "customize", label: "Customization", desc: "Appearance, profile & goals", icon: Palette },
  { key: "reminders", label: "Reminders & Alarm", desc: "Daily reminders & sleep alarm", icon: AlarmClock },
  { key: "data", label: "Backup & Data", desc: "Export, restore & reset", icon: ArchiveRestore },
];
  ...
  if (!tab) {
    return (
      <div className="px-5 pt-12 pb-4">
        ...
        <p className="text-xs text-slate-500 mb-5">Make this app yours. Customize everything.</p>
          ...
          {TABS.map((t) => {
            ...
            return (
              <button
                ...
                className="w-full flex items-center gap-4 px-4 py-4 rounded-2xl bg-white/5 border border-white/10 text-left hover:bg-white/8 transition-colors group"
              >
                <div className="w-10 h-10 rounded-xl bg-indigo-500/15 border border-indigo-400/20 flex items-center justify-center shrink-0">
                  <Ic className="w-5 h-5 text-indigo-300" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-white">{t.label}</p>
                  <p className="text-[11px] text-slate-500">{t.desc}</p>
                </div>
                <ChevronLeft className="w-4 h-4 text-slate-600 rotate-180 group-hover:text-slate-400 transition-colors" />
              </button>
            );
          })}
        </div>
  ...
  return (
    <div className="px-5 pt-12 pb-4">
      ...
      {tab === "ai" && (<>
        ...
        {/* Your Own Characters — inside AI tab */}
        <Section icon={Users} title="Your Own Characters">
          <p className="text-xs text-slate-400 mb-3 leading-relaxed">
          ...
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-semibold text-slate-300">{characters.length} character{characters.length === 1 ? "" : "s"}</p>
            <button
      ...
      {tab === "data" && (<>
        ...
        <div className="mt-6 mb-4">
          <button
            onClick={async () => {
              if (!confirm("This will permanently delete ALL your data — streak, check-ins, relapse history, sleep log, chats, and characters. This cannot be undone. Are you sure?")) return;
              try {
